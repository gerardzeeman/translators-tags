#!/usr/bin/env python3
"""Load parse_canones_nl.py's JSONL into the `translation` table, matching
each row to its Latin segment by ref (work slug 'canones-dordraceni').

Canones voorwoord.1 has no row in the JSONL (see parse_canones_nl.py) and
is simply left without a Dutch translation -- not an error.

    python scripts/load_canones_nl.py canones_nl.jsonl
